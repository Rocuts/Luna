...
// ─── Utilidad: transcripción → texto plano para reflexión ─────────────────────
...
function TranscriptPanel({ entries }: { entries: TranscriptEntry[] }) {
    ...
    useEffect(() => {
        ...
        if (el) {
            // Scroll instantáneo al fondo: evita el jitter de múltiples
            // animaciones 'smooth' encadenadas durante el streaming
    ...
    return (
        <div ref={containerRef} className="w-full max-w-2xl glass-panel rounded-2xl sm:rounded-3xl p-3 sm:p-5 max-h-44 sm:max-h-56 overflow-y-auto space-y-2.5 sm:space-y-3">
            {entries.map((entry) => (
                ...
                >
                    <div
                        className={`max-w-[88%] sm:max-w-[82%] px-3 py-2.5 sm:px-4 sm:py-3 rounded-2xl text-sm sm:text-base font-medium leading-relaxed ${
                            entry.role === 'user'
...
/** Pantalla cuando el usuario denegó el micrófono */
                ...
                <div className="space-y-3">
                    ...
                    {isInsecureContext && (
                        <p className="text-sm sm:text-base text-orange-900/70 leading-relaxed">
                            Si abriste la app por IP local (`http://192.168.x.x`), usa `http://localhost:3000` en este mismo equipo o publica con HTTPS.
                        </p>
                    )}
                </div>
                <button
...
/** Pantalla de reconexión compasiva */
...
/** Banner no-intrusivo de desconexión */
...
/** Indicador sutil de que la sesión se está guardando en memoria */
        ...
        <div className="flex flex-col items-center justify-center min-h-[100dvh] p-4 sm:p-6 gap-6 sm:gap-10">
            ...
            <div className="flex flex-col md:flex-row gap-4 sm:gap-5 w-full max-w-2xl">
                ...
                <button
                    ...
                    aria-label="Charla Libre: conversación libre con Luna sobre cualquier tema"
                    className={[
                        'flex-1 glass-panel rounded-[1.65rem] sm:rounded-[2rem] p-5 sm:p-8',
                        'flex flex-col items-center text-center gap-4',
                        'min-h-[190px] sm:min-h-[220px]',
                        'border-2 border-transparent',
                        'hover:border-aurora-500/40 hover:shadow-xl hover:scale-[1.02]',
                        'active:scale-[0.98]',
                        'transition-all duration-300',
                        'focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-aurora-500 focus-visible:ring-offset-2',
                        'cursor-pointer',
                    ].join(' ')}
                >
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm sm:text-base font-medium text-orange-900/70 leading-relaxed">
                            Conversa libremente con Luna sobre cualquier tema
                        </p>
                    </div>
                </button>
                ...
                <button
                    ...
                    aria-label="Clase Guiada: sesión estructurada con 3 preguntas de vocabulario y corrección inmediata"
                    className={[
                        'flex-1 glass-panel rounded-[1.65rem] sm:rounded-[2rem] p-5 sm:p-8',
                        'flex flex-col items-center text-center gap-4',
                        'min-h-[190px] sm:min-h-[220px]',
                ...
                >
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm sm:text-base font-medium text-orange-900/70 leading-relaxed">
                            Practica con 3 preguntas guiadas y corrección inmediata
                        </p>
                    </div>
                </button>
            </div>
...
export default function ChatInterface() {
    ...
    // Estado de reflexión: true mientras se llama a /api/reflect al final de sesión
    const [isReflecting, setIsReflecting] = useState(false);

    // Álbum de progreso visual
    const [showAlbum, setShowAlbum] = useState(false);

    // Ref espejo del transcript: siempre actualizado, seguro para capturar
    // en handleStop y en el cleanup de useEffect sin problemas de stale closure
    const transcriptRef = useRef<TranscriptEntry[]>([]);
    useEffect(() => {
        transcriptRef.current = transcript;
    }, [transcript]);

    // Ejecuta la reflexión y guarda el perfil actualizado en localStorage.
    // Retorna silenciosamente si la transcripción está vacía o la API falla.
    const runReflection = useCallback(async (entries: TranscriptEntry[]) => {
    ...
    // ── Token efímero con contexto de perfil ──────────────────────────────────
    ...
    // ── Registro de callbacks y limpieza al desmontar ─────────────────────────

    useEffect(() => {
        let mounted = true;

        realtimeService.setCallback((state) => {
            if (mounted) setSessionState(state);
        });
        realtimeService.setTranscriptCallback((entries) => {
            if (mounted) setTranscript(entries);
        });

        return () => {
            mounted = false;
            // Reflexión best-effort al desmontar (p.ej. navegación): fire-and-forget
            const entries = transcriptRef.current;
            if (entries.length > 0) {
                runReflection(entries).catch(() => {});
            }
    ...
    const handleStart = async () => {
        // Haptic feedback sutil al iniciar sesión (mejora UX en dispositivos móviles)
        if (navigator.vibrate) navigator.vibrate(15);

        // Si el audio estaba bloqueado por iOS, reintentar con este gesto
    ...
    const handleStop = async () => {
        // Capturar transcript ANTES de disconnect(), que lo vacía
        ...
        if (entriesToReflect.length > 0) {
            ...
            await runReflection(entriesToReflect);
    ...
    if (selectedMode === null) return <ModeSelectionScreen onSelect={setSelectedMode} />;

    // ── Cálculos visuales ─────────────────────────────────────────────────────

    // micVolume ya viene normalizado 0-1 desde el hook; escala max ±15%
    ...
    return (
        <>
        {showAlbum && <ProgressAlbum onClose={() => setShowAlbum(false)} />}
        ...
        <div className="flex flex-col items-center min-h-[100dvh] p-3 sm:p-8 gap-4 sm:gap-6 pb-[max(1rem,env(safe-area-inset-bottom))]">
            ...
            {/* ── Badge de guardado de memoria ────────────────────────────── */}
            {isReflecting && <SavingMemoryBadge />}

            {/* ── Transcripción (se desliza desde arriba) ──────────────────── */}
            ...
            {/* ── Zona central: estado + orbe + mensaje ─────────────────────── */}
            <div className="flex-1 flex flex-col items-center justify-center gap-5 sm:gap-8 w-full max-w-2xl">
                ...
                >
                    {/* Aura exterior cuando está escuchando */}
                    {isListening && (
                        <>
                    ...
                    `}>
                        {isConnecting ? (
                            <Loader2
                                className="w-16 h-16 sm:w-20 sm:h-20 text-white animate-spin opacity-80"
                                strokeWidth={1.5}
                            />
                        ) : isListening ? (
                            <Square
                                className="w-12 h-12 sm:w-16 sm:h-16 text-white fill-white opacity-90 transition-transform group-hover:scale-90"
                            />
                        ) : (
                            <Mic
                                className="w-16 h-16 sm:w-24 sm:h-24 text-white opacity-90 transition-transform group-hover:scale-110 drop-shadow-md"
                                strokeWidth={1.5}
                            />
                        )}
                ...
                {/* Mensaje empático de bienvenida */}
                <div className={`text-center max-w-sm px-2 transition-all duration-700 ${
                    ...
                    {/* Botones de acción: logros + cambiar modo */}
                    <div className="mt-6 sm:mt-8 flex flex-col items-center gap-3 w-full">
                        <button
                            onClick={() => setShowAlbum(true)}
                            aria-label="Ver mis logros desbloqueados"
                            className="w-full sm:w-auto px-6 sm:px-7 py-3 sm:py-4 bg-white/70 backdrop-blur-sm border border-orange-200 rounded-2xl font-bold text-base sm:text-lg text-orange-900 hover:bg-white hover:shadow-md active:scale-95 transition-all shadow-sm focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-400"
                        >
                            Ver mis logros ⭐
                        </button>
                        <button
                            onClick={handleChangeMode}
                            aria-label="Volver a seleccionar modo de práctica"
                            className="px-5 py-3 min-h-[48px] text-sm sm:text-base font-medium text-orange-900/60 hover:text-orange-900 hover:bg-white/40 rounded-xl transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400"
                        >
                            ← Cambiar modo
                        </button>
                    </div>
